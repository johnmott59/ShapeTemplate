...
namespace ShapeTemplateLib
{
    public class Utilities
    {
        ...
        public static bool GetXMLProperty(XElement ele,string PropertyName, out XElement xValue, out string message)
        {
            message = "";

            xValue = ele.Elements("xmlproperty")
                .Where(m => m.FirstAttribute.Name == "name" 
                && m.FirstAttribute.Value == PropertyName.ToLower())
                .FirstOrDefault();

            if (xValue == null)
            {
                message = $"Expected xml property {PropertyName}";
                return false;
            }
